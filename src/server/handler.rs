    ...
    where H: Handler<SonicMessage, Option<SonicMessage>> + Reset,
{
    handler: H,
    epfd: EpollFd,
    input_buffer: &'b mut ByteBuffer,
    output_buffer: &'b mut ByteBuffer,
    is_readable: bool,
    is_writable: bool,
    max_msg_size: usize,
    closed_write: bool,
    ...
    where H: Handler<SonicMessage, Option<SonicMessage>> + Reset,
{
    ...
    fn try_read(&mut self, fd: RawFd) -> Result<()> {
        ...
        if self.is_readable && !self.input_buffer.is_writable() {
            ...
            trace!("SonicHandler::try_read(): increased input_buffer size by {}",
                   additional);
    ...
    fn buffer(&mut self, msg: SonicMessage) -> Result<()> {
        match msg.to_buffer(self.output_buffer) {
            Err(Error(ErrorKind::BufferTooSmall(msg), _)) => {
                ...
                trace!("SonicHandler::buffer(): increased output_buffer size by {}",
                       additional);
                self.buffer(msg)
            }
            Ok(_) => Ok(()),
    ...
    where H: Handler<SonicMessage, Option<SonicMessage>> + Reset,
{
    fn reset(&mut self, epfd: EpollFd) {
        ...
        self.output_buffer.clear();
        self.handler.reset(epfd);
    }
}

impl<'b, H> Handler<MuxEvent, MuxCmd> for SonicHandler<'b, H>
    where H: Handler<SonicMessage, Option<SonicMessage>> + Reset,
{
    fn ready(&mut self, event: MuxEvent) -> MuxCmd {
        ...
        loop {
            match or_complete!(SonicMessage::from_buffer(&mut *self.input_buffer), self, fd) {
                Some(msg) => {
                    if let Some(res) = self.handler.ready(msg) {
                        trace!("handler.ready(): {:?}", res);
...
mod tests {
    ...
    fn new_handler<'a>(
        input_buffer: &'a mut ByteBuffer,
        output_buffer: &'a mut ByteBuffer
    ) -> SonicHandler<'a, TestHandler> {
        let epfd: EpollFd = EpollFd::new(0);
        SonicHandler::new(input_buffer, output_buffer, epfd, TestHandler)
    }

    fn setup_socketpair() -> (RawFd, RawFd, MuxEvent, MuxEvent, MuxEvent, MuxEvent) {
        let (s1, s2) = socketpair(AddressFamily::Unix, SockType::Stream, 0, SockFlag::empty())
            .unwrap();
        let readable = MuxEvent {
            fd: s1,
            kind: EPOLLIN,
        };
        let writable = MuxEvent {
            fd: s1,
            kind: EPOLLOUT,
        };
        let half_close = MuxEvent {
            fd: s1,
            kind: EPOLLRDHUP,
        };
        let close = MuxEvent {
