...
use super::{ResultExt, Error, Result, ErrorKind};
...
pub fn write_config(config: &ClientConfig, path: &PathBuf) -> Result<()> {
  debug!("overwriting or creating new configuration file with {:?}", config);
...
pub fn build(alias: String, mut sources: BTreeMap<String, Value>, auth: Option<String>,
             raw_query: String)
             -> Result<SonicMessage> {
